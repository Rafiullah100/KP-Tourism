...
internal struct InputContainerViewConstants {
    static let maxInputMessageContainerViewHeight: CGFloat = 200.0
...
class KeyboardInputAccessoryView: UIView {
    ...
    var delegate: KeyboardInputAccessoryViewProtocol?
    ...
    @IBAction func send(_ sender: UIButton) {
        delegate?.send(textView: self.inputTextView)
        dismissKeyboard()
    }
...
//        self.inputTextView.text = nil // we clean our keyboard
    ...
    @objc private func keyboardWillShow(notification: NSNotification) {
        ...
        if let scrollView = self.hostViewController?.scroll(), let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
    ...
    @objc private func keyboardWillHide(notification: NSNotification) {
        self.hostViewController?.scroll().contentInset = .zero
        self.hostViewController?.scroll().scrollIndicatorInsets = .zero
    }
}
