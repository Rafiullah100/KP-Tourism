...
class EventsViewController: BaseViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let modelObject = DataManager.shared.eventModelObject,
           let index = eventsArray.firstIndex(where: { $0.id == modelObject.id }) {
            eventsArray[index] = modelObject
    ...
    private func loadData(){
        if exploreDistrict != nil {
            ...
            fetch(parameters: ["district_id": exploreDistrict?.id ?? 0, "uuid": UserDefaults.standard.uuid ?? "", "limit": limit, "page": currentPage])
        }
        else if attractionDistrict != nil{
            thumbnailTopLabel.text = attractionDistrict?.title
            thumbnailBottomLabel.text = exploreDistrict?.locationTitle
            thumbnail.sd_setImage(with: URL(string: Route.baseUrl + (attractionDistrict?.previewImage ?? "")))
            fetch(parameters: ["district_id": attractionDistrict?.districtID ?? 0, "user_id": UserDefaults.standard.uuid ?? "", "limit": limit, "page": currentPage])
        }
            ...
            fetch(parameters: ["district_id": archeology?.attractions.districtID ?? 0, "uuid": UserDefaults.standard.uuid ?? "", "limit": limit, "page": currentPage])
        }
            ...
            fetch(parameters: ["district_id": wishlistAttraction?.districtID ?? 0, "uuid": UserDefaults.standard.uuid ?? "", "limit": limit, "page": currentPage])
        }
        if wishlistDistrict != nil {
            ...
            fetch(parameters: ["district_id": wishlistDistrict?.id ?? 0, "uuid": UserDefaults.standard.uuid ?? "", "limit": limit, "page": currentPage])
        }
    }
